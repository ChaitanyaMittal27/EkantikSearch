...
FILTERED_VIDEOS_FILE = "initial_populate/filtered_videos.txt"
# 🔹 Failed videos log file
FAILED_DETAILS_FILE = "initial_populate/failed_details.txt"
...
logging.basicConfig(
    filename="initial_populate/population_log.txt",
    filemode="w",
    level=logging.INFO,
...
def fetch_video_description(video_id):
    ...
    try:
        ...
        log(f"❌ Error: Could not fetch description for video {video_id}")
        return None
...
def main():
    ...
    with open(FILTERED_VIDEOS_FILE, "r", encoding="utf-8") as infile:
        for line in infile:
            ...
            if len(parts) != 3:
                log(f"❌ Error: Invalid format in filtered_videos.txt: {line.strip()}")
                sys.exit(1)  # Stop execution if file format is incorrect

            video_id, video_date, video_title = parts
            video_url = f"https://www.youtube.com/watch?v={video_id}"

            log(f"\n🎬 Processing Video: {video_title} | ID: {video_id} | Date: {video_date}")


            # Fetch video description
            description = fetch_video_description(video_id)
            if description is None:
                log(f"⚠️ Skipping {video_title} due to missing description.")
            ...
            if not questions:
                log(f"⚠️ No questions found in {video_title}, skipping.")
            ...
            # Add this after collecting entries
            CHUNK_SIZE = 200
            for i in range(0, len(entries), CHUNK_SIZE):
                chunk = entries[i:i + CHUNK_SIZE]
                log(f"🔄 Inserting questions {i + 1} to {i + len(chunk)} for video #{extract_video_index(video_title)}")
                insert_batch(chunk)
    ...
    log("✅ All videos processed successfully! Data saved in the db.")
    log(f"📌 Failed video details logged in {FAILED_DETAILS_FILE}")
